...
## 8. Decisiones (D1–D17, sección 6.1 del prompt)
...
- **Addendum de la Etapa 7 (bug real de concurrencia en `kysely-pglite`, con impacto directo en el requisito de check-in sin duplicados)**: el test de integración "dos confirmaciones simultáneas de la misma persona → una sola fila" (sección 12.3 del prompt) falló al principio, pero no por un error en `lib/attendance/checkin.ts` — el `UNIQUE(meeting_id, person_id)` y el `catch` con re-consulta estaban bien. La causa real: `kysely-pglite` (`node_modules/kysely-pglite/dist/pglite-driver.js`) implementa `acquireConnection()` devolviendo un wrapper nuevo por cada llamada, pero **todos comparten la misma sesión real de PGlite** (un único cliente embebido, no un pool de conexiones de verdad). Cuando dos `db.transaction().execute()` quedan en vuelo al mismo tiempo, Kysely emite `BEGIN`/`INSERT`/`COMMIT` de cada transacción lógica sobre esa misma sesión compartida, y los dos bloques se intercalan: la segunda transacción no ve un `23505` (unique violation) limpio como debería, sino `current transaction is aborted, commands ignored until end of transaction block` — el error de la primera le contamina el estado a la segunda. Confirmado con una prueba aparte: **PGlite en sí mismo sí serializa bien** dos `.transaction()` propios (API nativa) sobre el mismo archivo; el problema es específico de cómo `kysely-pglite` emite `BEGIN`/`COMMIT` como SQL de texto plano sin serializar. **Corrección**: `lib/db/client.ts` ahora envuelve el `Driver` que devuelve `kysely-pglite` con un mutex asíncrono propio (`serializePGliteTransactions`) que solo deja una transacción en curso a la vez — se adquiere en `beginTransaction` y se libera en `commitTransaction`/`rollbackTransaction`. Esto **solo se aplica al dialecto PGlite**; contra Postgres real (`SUTECBA_DATABASE_URL`, vía `pg.Pool`) cada conexión del pool es una sesión de verdad y no hace falta (ni se aplica) este mutex. Test de regresión: el caso de concurrencia en `tests/integration/attendance.test.ts`. **Limitación residual conocida y aceptada**: el mutex serializa transacciones entre sí, pero una consulta suelta (sin `.transaction()`) emitida mientras hay una transacción PGlite en curso todavía comparte la misma sesión y en teoría podría ver estado no confirmado — no se encontró ningún caso real que dependa de esto en la app actual (las escrituras concurrentes relevantes ya pasan todas por `.transaction()`), así que no se resolvió para no sobre-invertir en un límite que Postgres real no tiene.
...
- **Elección** (Etapa 7): ambos modos (estático con ventana, y rotativo tipo TOTP) soportados con la misma validación HMAC, **rotativo por defecto**, ventana de 45s (`qr_rotation_seconds` en `app_settings`) con tolerancia a la ventana anterior. El secreto real de cada reunión **se deriva** (`HMAC(SUTECBA_QR_SECRET, meetingId + ":" + qr_secret_version)`), no se guarda aparte — "regenerar el QR" es incrementar `meetings.qr_secret_version`, sin necesitar una segunda columna de secreto. Implementado en `lib/attendance/qr.ts` (firma/verificación, sin acceso a base) y `lib/attendance/checkin.ts` (`resolveQrToken`, que sí compara contra la versión vigente en base — la firma sola no alcanza para detectar una regeneración, ver el módulo para el razonamiento completo).
- **Cómo revertir**: no aplica sin rediseñar el mecanismo de check-in.

### D12 — Panel en tiempo real
- **Elección**: *polling* contra Route Handlers protegidos que reutilizan las mismas funciones que usaría cualquier página normal — no hay una fuente de datos paralela para el polling. Cadencias elegidas en la Etapa 7: panel en vivo (`/api/reuniones/[id]/live`) cada 4 segundos, imagen del QR (`/api/reuniones/[id]/qr`) cada 20 segundos (la ventana rotativa dura 45s con tolerancia a la anterior, así que 20s deja margen de sobra sin generar tráfico innecesario).
...
### D17 — Formularios: identificación, deduplicación y versionado (Etapa 8)
- **Snapshot inmutable por publicación**: `forms`/`form_fields`/`form_actions` son el estado *editable* (lo que ve el constructor); publicar arma un JSON (`FormVersionSchema`, `lib/forms/version-schema.ts`) con campos + acciones + política de identificación/actualización en ese momento, y lo guarda en `form_versions.schema`. La página pública y el procesamiento de un envío **siempre** leen la versión publicada (`forms.published_version`), nunca las tablas editables — así seguir tocando el formulario después de publicarlo no reinterpreta respuestas ya recibidas ni las que alguien tiene a medio completar en el navegador. La resolución manual de un candidato a duplicado (`lib/forms/duplicates.ts`) va un paso más allá: usa la versión con la que se **envió ese formulario en particular** (`form_submissions.form_version`), no la vigente hoy, por si el formulario se re-publicó en el medio.
- **Identificación**: `identification_policy.matchFields` (subconjunto de `dni`/`email`/`phone`, configurable por formulario) define qué campos núcleo se usan para buscar a la persona ya existente (`lib/forms/matching.ts`). Si distintos campos apuntan a personas *distintas* (ambigüedad real, no un simple "no hay match"), nunca se elige una sola: se crea un `person_duplicate_candidates` por cada persona posible y el envío queda `needs_review` sin `person_id` asignado.
- **update_policy**: `fill_empty_only` aplica el envío al toque, pero solo completa columnas/`custom_fields` que la persona ya tenía vacíos (nunca pisa un dato cargado); `always_flag_for_review` no toca a la persona automáticamente ni con un único match: siempre pasa por la bandeja de revisión (`forms.review_duplicates`), que ofrece vincular (aplica el mismo merge que `fill_empty_only`), crear una persona nueva, o descartar.
- **Campos personalizados cierran el círculo**: `form_fields.person_field_mapping` acepta una columna núcleo (`first_name`, `last_name`, `dni`, `email`, `phone`, `birth_date`) o la clave de un `person_field_definitions` activo — en ese caso el valor normalizado se escribe en `people.custom_fields[clave]` (columna que ya existía desde la Etapa 2/3 pero sin ningún código que la usara). El catálogo de campos personalizados en sí (`lib/people/field-definitions.ts`, admin en `/administracion/campos-personalizados`, permiso nuevo `people.manage_custom_fields`) también se construyó en esta etapa porque sin él el mapeo a "campo personalizado" no tenía ninguna opción real para ofrecer.
- **Pendiente explícito**: Personas (la ficha y el alta manual) todavía no lee ni edita `custom_fields` — hoy el único camino de escritura es un envío de Formulario. Editar campos personalizados a mano desde Personas queda fuera de esta etapa.
- **Hallazgo real (tests de integración)**: "crear una persona nueva" desde la bandeja de duplicados, cuando el candidato matcheaba justamente por DNI, intentaba insertar una segunda persona con ese mismo DNI y rompía contra `people_dni_unique_idx` con un error crudo del driver. Es un caso real: un DNI no puede pertenecer a dos personas activas, así que "es una persona nueva" no tiene sentido cuando lo único que matcheó fue el DNI (si matchea por email/teléfono con un DNI distinto, sí es válido). Se corrigió chequeando el DNI *antes* de insertar en `lib/forms/apply.ts` (`createPersonFromSubmission`) y devolviendo un `FormApplyError` con un mensaje claro («ya existe una persona con el DNI…») en vez de dejar escapar la excepción del driver. Test de regresión: `tests/integration/forms.test.ts`, caso "con el mismo DNI que ya causó el match da un error claro, no un crash".
- **Cómo revertir**: el versionado es aditivo (agregar `form_versions` es lo que ya se hizo); la política de identificación/actualización es configuración por formulario, ajustable sin migración.

### D18 — Visualización (Etapa 9)
- **Elección**: un único módulo de consulta (`lib/analytics/queries.ts`, cuatro funciones — Personas/Asociaciones/Reuniones/Formularios) que agrega en SQL (nunca trae miles de filas a Node para contarlas ahí — lección de la sección 6.2) y que tanto `/visualizacion` como (parcialmente) `/dashboard` pueden consumir, para que nunca haya dos cálculos separados del mismo número que con el tiempo empiecen a discrepar. `components/charts/` son wrappers delgados de Recharts (`BarChartCard`, `LineChartCard`, `StatTile`) — Recharts se adapta (ya estaba previsto en la estructura de carpetas desde la Etapa 1), su contenido no se copia de la referencia.
- **Series mensuales sin huecos**: los últimos 12 meses se generan en código (no se infieren de los datos) para que un mes sin ningún registro aparezca en 0 en vez de faltar del todo — si no, el eje X de un gráfico de línea "saltearía" ese mes y se leería como si el eje tuviera menos meses de los que tiene.
- **Contenido territorial descartado a propósito**: la referencia tiene un módulo de Visualización equivalente, pero armado alrededor de comunas/verticales/semáforos — conceptos que no existen en el dominio de SUTECBA (R3). Se adaptó únicamamente el *patrón* (un módulo de datos compartido, series con huecos rellenados, wrappers de Recharts), con métricas propias: por organismo, por tipo de asociación, completitud de datos de Personas, tasa de asistencia y resultado de matching de Formularios.
- **Hallazgo real (corregido antes de mergear, sin llegar a producir un dato mal mostrado)**: la función que arma las claves "YYYY-MM" de los últimos 12 meses leía `getUTCMonth()` de `new Date()` directo, mientras que el lado SQL correctamente hace `AT TIME ZONE '-03:00'` antes de truncar a mes (mismo criterio que D15/`lib/datetime.ts`). Como Buenos Aires siempre está *detrás* de UTC, durante las primeras ~3 horas de cada mes en UTC (que en Buenos Aires todavía son el mes anterior) las dos claves habrían quedado desincronizadas y los registros del mes en curso se habrían mostrado como si no tuvieran mes. Corregido restando el offset fijo antes de leer año/mes en el lado JS, para que las dos mitades (SQL y JS) usen siempre el mismo criterio de zona horaria.
- **Permiso ajustado de paso**: al revisar el catálogo se notó que OPERADOR (el rol operativo de uso diario) no tenía `visualization.view` ni ningún `forms.*`, a pesar de tener alta/edición en Personas/Asociaciones/Reuniones — una asimetría que no tenía justificación de negocio documentada. Se agregó `visualization.view` y `forms.view/create/edit/publish/review_duplicates` a OPERADOR (no `forms.export_submissions`, igual que tampoco tiene `people.export`, por coherencia).
- **Cómo revertir**: no aplica sin cambiar de librería de gráficos; las métricas son consultas, agregar/quitar una no requiere migración.
